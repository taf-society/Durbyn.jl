...
function forecast(model::MeanFit, h::Int=10, level::Vector{Float64}=[80.0, 95.0], fan::Bool=false, bootstrap::Bool=false, npaths::Int=5000)
    meanx = model.mu
    s = model.sd
    lambda = model.lambda
    m = model.m
    ...
    if bootstrap
        e = collect(skipmissing(model.residuals)) .- mean(collect(skipmissing(model.residuals)))
        ...
        n = length(collect(skipmissing(model.fitted)))
        for i in 1:nconf
    ...
    if !isnothing(lambda)
        f = inv_boxcox.(f, lambda)
        lower .= inv_boxcox.(lower, lambda)
        upper .= inv_boxcox.(upper, lambda)
    end
