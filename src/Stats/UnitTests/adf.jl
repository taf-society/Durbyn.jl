...
# Notes
`ADF` exposes output-compatibility aliases shared across unit-root tests:
- `result.teststat` (primary statistic; equals `tau`)
- `result.stats` (named tuple with all reported statistics)
- `result.primary_cval` (critical values for the primary statistic)
- `result.critical_values` (named tuple keyed by statistic name)
- `result.lag_order`, `result.deterministic`, `result.coefficients`,
  `result.standard_errors`, `result.regression`, `result.residuals`
"""
struct ADF
    model::Symbol
    cval::Matrix{Float64}
...
# MacKinnon (2010) response surface coefficients for τ statistics.
# Formula: c(p, T) = β∞ + β₁/T + β₂/T² + β₃/T³
# Source: Tables 2 and 3 of the 2010 appendix to
#   MacKinnon, J. G. (2010). "Critical Values for Cointegration Tests."
#   Queen's Economics Department Working Paper No. 1227.
# Rows are [1%, 5%, 10%]; columns are [β∞, β₁, β₂, β₃].
const _MACKINNON_TAU_NC = [  # τ_nc (no constant), Table 2, N=1
    -2.56574  -2.2358   -3.627    0.0;
    -1.94100  -0.2686   -3.365   31.223;
    -1.61682   0.2656   -2.714   25.364
]
const _MACKINNON_TAU_C = [  # τ_c (with constant), Table 2, N=1
    -3.43035  -6.5393  -16.786  -79.433;
    -2.86154  -2.8903   -4.234  -40.040;
    -2.56677  -1.5384   -2.809    0.0
]
const _MACKINNON_TAU_CT = [  # τ_ct (with constant and trend), Table 3, N=1
    -3.95877  -9.0531  -28.428  -134.155;
    -3.41049  -4.3904   -9.036   -45.374;
    -3.12705  -2.5856   -3.925   -22.380
]

function _mackinnon_tau_cval(coeffs::Matrix{Float64}, T::Int)
    invT = 1.0 / T
    return [coeffs[i, 1] + coeffs[i, 2] * invT + coeffs[i, 3] * invT^2 + coeffs[i, 4] * invT^3
            for i in 1:size(coeffs, 1)]
end

# Phi-statistic critical values from Dickey & Fuller (1981, Table VI),
# as reported in Hamilton (1994, Tables B.6 and B.7).
# Unlike the τ statistics, no response surface is available for φ;
# values are tabulated at asymptotic and selected finite-sample sizes.
# Columns: [1%, 5%, 10%].  Indexed by _phi_row(T).
function _phi_row(T::Int)
    T < 25  && return 1
    T < 50  && return 2
    T < 100 && return 3
    T < 250 && return 4
...
const _PHI1_CVAL = [  # φ₁: drift model, H₀: (α, γ) = (0, 0)
    7.88  5.18  4.12;
    7.06  4.86  3.94;
    6.70  4.71  3.86;
    6.52  4.63  3.81;
...
const _PHI2_CVAL = [  # φ₂: trend model, H₀: (α, γ, δ) = (0, 0, 0)
    8.21  5.68  4.67;
    7.02  5.13  4.31;
    6.50  4.88  4.16;
    6.22  4.75  4.07;
...
const _PHI3_CVAL = [  # φ₃: trend model, H₀: (γ, δ) = (0, 0)
    10.61  7.24  5.91;
     9.31  6.73  5.61;
     8.73  6.49  5.47;
     8.43  6.49  5.47;
...
function _adf_critical_values(model::Symbol, n_obs::Int)
    if model == :none
        tau_cv = _mackinnon_tau_cval(_MACKINNON_TAU_NC, n_obs)
        return reshape(tau_cv, 1, 3), [:tau1]
    elseif model == :drift
        tau_cv = _mackinnon_tau_cval(_MACKINNON_TAU_C, n_obs)
        r = _phi_row(n_obs)
        phi1_cv = _PHI1_CVAL[r, :]
        C = vcat(reshape(tau_cv, 1, 3), reshape(phi1_cv, 1, 3))
        return C, [:tau2, :phi1]
    else
        tau_cv = _mackinnon_tau_cval(_MACKINNON_TAU_CT, n_obs)
        r = _phi_row(n_obs)
        phi2_cv = _PHI2_CVAL[r, :]
        phi3_cv = _PHI3_CVAL[r, :]
        C = vcat(reshape(tau_cv, 1, 3), reshape(phi2_cv, 1, 3), reshape(phi3_cv, 1, 3))
...
function Base.propertynames(::ADF; private::Bool=false)
    base = (
        ...
        :testreg, :res, :testnames,
        :lag, :lag_order, :deterministic, :teststat, :stats, :primary_cval, :critical_values,
...
function Base.getproperty(x::ADF, sym::Symbol)
    ...
    elseif sym === :lag_order
        return getfield(x, :lags)
    elseif sym === :deterministic
        return getfield(x, :model)
    elseif sym === :teststat
        return getfield(x, :tau)
    elseif sym === :stats
        return _adf_stats(x)
    elseif sym === :primary_cval
        return vec(getfield(x, :cval)[1, :])
    elseif sym === :critical_values
        statistic_names = getfield(x, :testnames)
        cval_matrix = getfield(x, :cval)
        return NamedTuple{Tuple(statistic_names)}(Tuple(vec(cval_matrix[i, :]) for i in eachindex(statistic_names)))
    elseif sym === :coefficients
        return getfield(x, :beta)
...
Augmented Dickey-Fuller unit root test (Dickey & Fuller, 1979; Said & Dickey, 1984).

The tested regression is:

- `model=:none`:  ``Δy_t = γ y_{t-1} + Σ c_j Δy_{t-j} + u_t``
- `model=:drift`: ``Δy_t = α + γ y_{t-1} + Σ c_j Δy_{t-j} + u_t``
- `model=:trend`: ``Δy_t = α + γ y_{t-1} + δt + Σ c_j Δy_{t-j} + u_t``

`tau` is the t-statistic on `γ`. For `:drift` and `:trend`, phi statistics are
computed as restricted-vs-full F-tests.

Finite-sample critical values for τ statistics are computed from the MacKinnon (2010)
response surface: ``c(p) = β_∞ + β_1/T + β_2/T^2 + β_3/T^3``
(Tables 2 and 3 of the 2010 appendix).  Critical values for φ statistics are from
Dickey & Fuller (1981), as tabulated in Hamilton (1994, Tables B.6–B.7).

# Keyword Arguments
- `model` / `type`: Deterministic component (`:none`, `:drift`, `:trend`).
- `lags`: Fixed lag order (nonneg integer).
- `k_max`: Max lag for automatic selection.
- `criterion` / `selectlags`: `:aic` or `:bic` for automatic lag selection; `:fixed` otherwise.
...
- Hamilton, J. D. (1994). *Time Series Analysis.* Princeton University Press. Tables B.6–B.7.
- MacKinnon, J. G. (2010). *Critical Values for Cointegration Tests.*
  Queen's Economics Department Working Paper No. 1227.
- Said, S. E. and Dickey, D. A. (1984). *Testing for Unit Roots in ARMA Models
