...
Normalizes by observed total adoption (natural scale for market potential).
...
- `cleanlead::Bool=true`: If true, remove leading zeros before fitting.
...
- `w::Union{Nothing, NamedTuple}=nothing`: Fixed parameter values. Use `nothing` values
  ...
  p at 0.03 while estimating m and q. All parameters can be fixed.
...
function fit_diffusion(y::AbstractVector{<:Real};
    ...
    if is_linearize
        total_adoption = Y_clean[end]
        if init_params.m < total_adoption
            init_dict_check = _params_to_dict(init_params)
            init_dict_check[:m] = total_adoption * 1.1
    ...
    y_total = Y_clean[end]
    mscal_factor = mscal && y_total > 0 ? y_total : one(T)
