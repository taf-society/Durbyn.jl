...
function _update_innovation_covariance!(
    innovation_covariance::Matrix{Float64},
    theta::AbstractVector,
    r::Int,
)
    fill!(innovation_covariance, 0.0)
    innovation_covariance[1, 1] = 1.0

    q_effective = min(length(theta), r - 1)
    @inbounds for i in 1:q_effective
        theta_i = Float64(theta[i])
        row_i = i + 1
        innovation_covariance[row_i, 1] = theta_i
        innovation_covariance[1, row_i] = theta_i

        for j in 1:i
            theta_j = Float64(theta[j])
            row_j = j + 1
            covariance_value = theta_i * theta_j
            innovation_covariance[row_i, row_j] = covariance_value
            innovation_covariance[row_j, row_i] = covariance_value
        end
    end
    return nothing
end
...
function update_arima(mod::Union{ArimaStateSpace,SARIMASystem}, phi, theta; kappa::Float64=1e6)
    ...
    # Update process noise covariance V = R * R' where
    # R = [1, θ₁, ..., θ_{r-1}, 0, ..., 0]
    _update_innovation_covariance!(mod.V, theta, r)
