...
This implementation follows Durbyn's ARIMA mathematical specification and
state-space formulation documented in `docs/src/arima.md`, with support for a
**drift** term and optional **Box-Cox transformation** with mean (bias)
adjustment on the back-transformed scale. You can also pass a previously fitted
model and re-apply it to new data `y` without re-estimating parameters.
...
- `include_constant=true` is a convenience that sets sensible defaults for
  `include_mean`/`include_drift` according to the differencing structure used
