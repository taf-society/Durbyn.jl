...
@testset "ARIMAX parity: multiple xreg ARIMA(1,0,1) + intercept" begin
    ...
    coefs = vec(fit.coef.data)
    expected_coefs = [0.9756956823927192, 0.42366076611286196,
                      129.34637155679732, -2.2316067973917244, -5.6635895770449975]
    @test coefs ≈ expected_coefs rtol=COEF_RTOL

    @test fit.sigma2 ≈ 1022.5879904964519 rtol=STAT_RTOL
    @test fit.loglik ≈ -702.4425497088415 rtol=STAT_RTOL

    fc = forecast(fit; xreg=NEWXREG_MULTI)
    expected_fc = [456.7147083557359, 449.89128769211527, 443.1395001727973,
                   436.4310914257104, 429.74240905340815, 423.05490638192055,
                   416.3554774311506, 409.6366142295316, 402.8963839700765,
                   396.1382299001368, 389.3706060142922, 382.6064613499111]
    @test fc.mean ≈ expected_fc rtol=FC_RTOL

    expected_lower80 = [415.73334243376064, 379.4057392122455, 353.1448657647125,
                        331.171791090606, 311.7725416987947, 294.14402908750486,
                        277.8295356887197, 262.5401003380884, 248.08105249371917,
                        234.3164507524832, 221.14973424552898, 208.51216912829406]
    expected_upper80 = [497.6960742777112, 520.376836171985, 533.134134580882,
                        541.6903917608148, 547.7122764080216, 551.9657836763363,
                        554.8814191735814, 556.7331281209747, 557.7117154464338,
                        557.9600090477904, 557.5914777830554, 556.7007535715281]
    expected_lower95 = [394.0391177639554, 342.0929437430055, 305.5045840414752,
                        275.45088333941317, 249.32306620960006, 225.9027327936902,
                        204.4983317342615, 184.6719095466505, 166.12676649522578,
                        148.65317539995684, 132.09898388132166, 116.35221778901348]
    expected_upper95 = [519.3902989475165, 557.6896316412251, 580.7744163041193,
                        597.4112995120076, 610.1617518972163, 620.2070799701509,
                        628.2126231280397, 634.6013189124126, 639.6660014449272,
                        643.6232844003168, 646.6422281472628, 648.8607049108087]
    @test fc.lower[:, 1] ≈ expected_lower80 rtol=PI_RTOL
    @test fc.upper[:, 1] ≈ expected_upper80 rtol=PI_RTOL
    @test fc.lower[:, 2] ≈ expected_lower95 rtol=PI_RTOL
    @test fc.upper[:, 2] ≈ expected_upper95 rtol=PI_RTOL
end
